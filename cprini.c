...
main()
{
  int n,j,i,m,k,a,itype,iks[100],i1,i2,dum1,dum2;
  double xk,xkp1,xks[100],errs[100];
  float sks[100];



            cprini("stdout","data13");
            //cprini("data17","stdout");
  //        cprini("    ","data13");
...
*/
        ...
        cprin_start_stop(0,1);

        cprin("finally, sks=",sks,n);
        cprinf("finally, iks=",iks,n);

        cprin_start_stop(1,1);

        cprina("message =","this is a sentance with words in it",20);
...
*/
    if(itype==11) {
        ...
        if(!str17) goto jump1;
        if(str17 != "stdout" ) st1=fopen(str17,"w");
        if(str17== "stdout") st1=stdout;

    jump1:

        if(!str27) goto jump2;
        if(str27 != "stdout" ) st2=fopen(str27,"w");
        if(str27== "stdout") st2=stdout;
    jump2:
